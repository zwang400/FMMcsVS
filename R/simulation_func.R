                ...
                for(m in 1:k){
                    ...
                    for(d in 1:D){
                        C_md <- sqrt(tau[d]/(tau[d] + lambda*sum(X[V_m, d]^2)))*exp(-0.5*tau[d]*mu^2)*
                            exp(((mu*tau[d] + lambda*sum((y[V_m] - alpha[m] - X[V_m, -d] %*% Beta[m, -d])*
...
simulation_split <- function(W, Z, y, prior = "Dirichlet", SS = TRUE, N = 1e5, gamma_hyperprior = TRUE,
    ...
    if(prior == "Dirichlet"){
        ...
        group_member <- function(m, Beta, zeta, psi, lambda, alpha, eta){
            llkhd <- rep(0, n)
            for(i in 1:n){
                llkhd_x <- 0
                for(j in 1:d2){
                    llkhd_x <- llkhd_x + dnorm(Z[i,j], zeta[m, j], sqrt(1/eta), log = T)
                }
                llkhd[i] <- dnorm(y[i], alpha[m] + W[i, ] %*% psi + Z[i,]%*%Beta[m,], sqrt(1/lambda), log = T) + llkhd_x
            }
            llkhd
        }
